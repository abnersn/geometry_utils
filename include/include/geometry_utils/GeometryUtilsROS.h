...
inline geometry_msgs::PoseWithCovariance ToRosPoseWithCovariance(
  const Transform3& trans,
  const Eigen::Matrix<float, 6, 6>& cov_mat) {
  
  geometry_msgs::PoseWithCovariance cov_msg;
  geometry_msgs::Pose pose_msg;
  
  boost::array<double, 36> cov_vec;
  for(int i = 0; i < cov_mat.rows(); i++) {
    for(int j = 0; j < cov_mat.cols(); j++) {
      cov_vec[i * 6 + j] = cov_mat(i, j);
    }
  }

  pose_msg.position = ToRosPoint(trans.translation);
  pose_msg.orientation = ToRosQuat(RToQuat(trans.rotation));
  cov_msg.pose = pose_msg;
  cov_msg.covariance = cov_vec;
